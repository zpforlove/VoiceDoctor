...
class HybridVAD:
    ...
    def __init__(
            ...
            # ==================== [ 解决幻觉 ] ====================
            trim_thr: float = 0.003,  # 原为 0.002, 提高裁剪阈值，配合能量门限
            tail_keep_ms: int = 0,
    ):
        self.sr = sr_out
...
def main():
    ...
    parser.add_argument("--audio", default="./wav/demo1.mp3", help="输入音频路径（建议 ≤ 40 分钟）")
    ...
    # VAD 切分参数
    parser.add_argument("--silence", type=float, default=0.3, help="静音累计阈值（秒），达到则结束一句")
    parser.add_argument("--min-utt", type=float, default=0.2, help="导出的最短句长（秒）")
    parser.add_argument("--max-utt", type=float, default=10.0, help="导出的最长句长（秒），防止超长不切")

    parser.add_argument("--spk-threshold", type=float, default=0.4, help="声纹匹配阈值（余弦相似度）")

    parser.add_argument("--use-deepseek", action="store_true", default=False,
